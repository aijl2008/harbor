# Installation and Configuration Guide 安装和配置指南

Harbor can be installed by one of two approaches 可采用以下两种方法之一:
...
- **System level parameters**: 需要在配置文件中设置这些参数。它们将生效，如果用户更新他们在```harbor.yml``` 。然后运行安装。```install.sh```脚本重新安装港口。
...
- **User level parameters**: 这些参数可以在Web Portal第一次启动harbor之后更新。特别是，您必须在Harbor中注册或创建任何新用户之前设置所需的**auth_mode**。当系统中有用户(除了默认的管理用户)时，不能更改**auth_mode**。

The parameters are described below - note that at the very least, you will need to change the **hostname** attribute.
参数如下所述——注意，至少需要更改**hostname**属性。

##### Required parameters

- **hostname**: The target host's hostname, which is used to access the Portal and the registry service. It should be the IP address or the fully qualified domain name (FQDN) of your target machine, e.g., `192.168.1.10` or `reg.yourdomain.com`. _Do NOT use `localhost` or `127.0.0.1` or `0.0.0.0` for the hostname - the registry service needs to be accessible by external clients!_ 目标主机的主机名，用于访问门户和注册中心服务。它应该是目标机器的IP地址或完全限定域名(FQDN)，例如“192.168.1.10”或“reg.yourdomain.com”。不要使用“localhost”或“127.0.0.1”或“0.0.0.0”作为主机名——注册中心服务需要被外部客户端访问
...
- **https**: The protocol used to access the Portal and the token/notification service. 用于访问门户和令牌/通知服务的协议。  If Notary is enabled, has to set to _https_.如果启用了Notary，则必须将其设置为_https_。
...
- **storage_service**: By default, Harbor stores images and chart on your local filesystem. 默认情况下，Harbor将图像和图表存储在本地文件系统中。 In a production environment, you may consider use other storage backend instead of the local filesystem, like S3, OpenStack Swift, Ceph, etc. 在生产环境中，您可以考虑使用其他存储后端而不是本地文件系统，如S3、OpenStack Swift、Ceph等。These parameters are configurations for registry.这些参数是注册表的配置。
...
#### Finishing installation and starting Harbor 完成安装并启动Harbor

Once **harbor.yml** and storage backend (optional) are configured, install and start Harbor using the `install.sh` script.  一旦配置了**harbor.yml**和存储后端(可选)，使用`install.sh`安装并启动Harbor。Note that it may take some time for the online installer to download Harbor images from Docker hub.注意，在线安装程序可能需要一些时间才能从Docker hub下载端口图像。

##### Default installation (without Notary/Clair)

Harbor has integrated with Notary and Clair (for vulnerability scanning). Harbor已与Notary和Clair(漏洞扫描)集成。However, the default installation does not include Notary or Clair service.但是，默认安装不包括Notary或Clair服务。
...
要安装带有Notary的Harbor，请在运行`install.sh`时添加一个参数:
```sh
...
**Note**: 安装Notary时，参数**ui_url_protocol**必须设置为 "https".。要配置HTTPS，请参考以下部分。
...
**Note**: 如果要安装Notary、Clair和chart repository service，必须在同一个命令中指定所有参数
...
Harbor does not ship with any certificates, and, by default, uses HTTP to serve requests. 
Harbor不附带任何证书，默认情况下使用HTTP来服务请求。
While this makes it relatively simple to set up and run - especially for a development or testing environment - it is **not** recommended for a production environment.  
虽然这使得设置和运行相对简单—特别是对于开发或测试环境—但是不推荐在生产环境中使用它。
To enable HTTPS, please refer to **[Configuring Harbor with HTTPS Access](configure_https.md)**.
